...
// tensor_set.cc
//
// Copyright (C) 2003-2005,2012 Walter Dehnen
//
...
#undef T4i
  ...
  //
  tNX inline symset3D<N,X>& symset3D<N,X>::ass_out_prd(falcONVec<3,X> const&v)
  { meta3D::op<N>::as(a,pX(v)); return*this; }
  tNX inline poles3D<N,X>& poles3D<N,X>::ass_out_prd(falcONVec<3,X> const&v)
  ...
  poles3D<N,X>::add_body(falcONVec<3,X> const&v, X const&m)
  ...
  //
  tNX inline poles3D<N,X>&
  poles3D<N,X>::add_cell(falcONVec<3,X> const&v, X const&m, poles3D<N,X>const&p)
  {
  ...
  //
  tNX inline void
  symset3D<N,X>::ass_inn_prd(falcONVec<3,X> const&v, symset3D<N-1,X>&l) const
...
#  define __SD(I,J)  sign_times<I>(D[I])
	...
	a[ 3] = v[2]*t;
      ...
      tX sv ass(symset3D<1,X>&C, const X*v, D_ARG)
      { __ass((X*)(C),v,ARG_D); }
	...
	a[ 3]+= v[2]*t;
      ...
      tX sv add(symset3D<1,X>&C, const X*v, D_ARG)
      { __add((X*)(C),v,ARG_D); }
    };
    //
    tm<> struct __grav<2> {
	...
	a[ 9] = t*v[2] - __D(1,J);
      ...
      tX sv ass(symset3D<2,X>&C, const X*v, D_ARG)
      { __ass((X*)(C),v,ARG_D); }
      tX sv __add(X*a, const X*v, D_ARG)
      {
	a[ 0]+= __D(0,J);
	...
	a[ 9]+= t*v[2] - __D(1,J);
      ...
      tX sv add(symset3D<2,X>&C, const X*v, D_ARG)
      { __add((X*)(C),v,ARG_D); }
    };
    //
    tm<> struct __grav<3> {
	...
	a[14] =-__D(3,J)*v[0]*v[1]*v[2];
      ...
      tX sv ass(symset3D<3,X>&C, const X*v, D_ARG)
      { __ass((X*)(C),v,ARG_D); }
      tX sv __add(X*a, const X*v, D_ARG)
      {
	a[ 0]+= __D(0,J);
	...
	a[14]-= __D(3,J)*v[0]*v[1]*v[2];
      ...
      tX sv add(symset3D<3,X>&C, const X*v, D_ARG)
      { __add((X*)(C),v,ARG_D); }
    };
#undef D_ARG
#undef ARG_D
...
#undef __SD
...
#ifdef falcON_SSE_CODE
  tNX inline void
  set_dPhi(symset3D<N,X>&C, falcONVec<3,X> const&v,
	   const fvec4 D[N+1], int const&J)
  { meta3D::__grav<N>::ass(C,static_cast<const X*>(v),D,J); }
  //
  tNX inline void
  add_dPhi(symset3D<N,X>&C, falcONVec<3,X> const&v,
	   const fvec4 D[N+1], int const&J)
 ...
 tNX inline void 
  add_C_C2C(symset3D<N,X>&c, symset3D<N,X> const&f, poles3D<N-1,X> const&m)
  {
    ...
    //N = 3, X = real, c -> A->Coeffs -> Fn, f-> D_phi, m -> multipoles coeffs 
    // meta3D::__c_cell<N>::ad(c,f,m); //__c_cell<ORD>::ad(c,f,m) -> ORD = 3 
    // static const int  P=2; //P= ORD -1
    //printf("real %d\n", c[0]); 
     for(int i =0; i < 20; i++)
      {
	c[i] += f[i];
    ...
    //    c[0] +=  m[0]*f[4] + m[1]*f[5] +   m[2]*f[6] +  m[3]*f[7] + m[4]*f[8] +  m[5]*f[9];
    c[0] +=  f[4]*m[0]  + f[9]*m[5]  + f[7]*m[3] ; 
    c[0] +=  2.0f*(f[5]*m[1]  + f[6]*m[2]  + f[8]*m[4]) ;
    //   printf(" C->0 %22.15e \n C->5 %22.15e\n  C->7 %22.15e\n  C->1 %22.15e\n  C->2 %22.15e\n  C->4 %22.15e\n",f[4]*m[0],f[9]*m[5],f[7]*m[3],f[5]*m[1],f[6]*m[2],f[8]*m[4]);
    //    c[1] +=  m[0]*f[10] + m[1]*f[11] + m[2]*f[12] +  m[3]*f[13] + m[4]*f[14] +  m[5]*f[15]; 
    c[1] +=  f[10]*m[0] + f[15]*m[5] + f[13]*m[3] ; 
    c[1] +=  2.0f*(f[11]*m[1] + f[12]*m[2] +  f[14]*m[4]);
    //c[2] +=  m[0]*f[11] + m[1]*f[13] + m[2]*f[14] +  m[3]*f[16] + m[4]*f[17] +  m[5]*f[18];
    c[2] +=  m[0]*f[11] + m[3]*f[16] + m[5]*f[18];
    c[2] += 2.0f*(m[1]*f[13] + m[2]*f[14] + m[4]*f[17]);
    
    // c[3] +=  m[0]*f[12] + m[1]*f[14] + m[2]*f[15] +  m[3]*f[17] + m[4]*f[18] +  m[5]*f[19];
    c[3] +=  m[0]*f[12] +   m[3]*f[17] +   m[5]*f[19];
    c[3] +=  2.0f*(m[1]*f[14] +  m[2]*f[15] + m[4]*f[18]) ;
    // for(int i =0; i < 4; i++)
    //   {
    // 	printf("c[%i] = %22.15e\n ",i,c[i]);
	...
	printf("c[%i] = %22.15e\n ",i,c[i]);
      ...
      exit(0);*/
    // //c += f;
    //     __coef_m<ORD,ORD,2,P,1>::job(c,f,m); -> ORD = 3, P = 2// __coef_m<ORDR,N,M,M,S> 3,3,2,2,1
    //sci KU = 1;  // sci KU = __min(N,ORDR-M), P=ORDR-1; -> KU = min( 3,1) = 1 , P = 2
    //	__coef_k<ORDR,N,M,0,KU, S>::job(c,f,m.template pole<M>()); __coef_k<ORDR,N,M,K,KU,1> 3,3,2,0,1,1
    // //f.template tensor<2>().add_inn_prd(m.template pole<2>(), c.template tensor<0>());
    //3,3,2,1,1,1
    //__coef_k<ORDR,N,M,K+1,KU,1>::job(c,f,p);
    // //f.template tensor<3>().add_inn_prd(m.template pole<2>(), c.template tensor<1>());
 ...
 {// meta3D::__c_body<N>::ad(c,f,m);
     int P=2;
     symset3D<1,X> cc=f.template subset<1>();
     // meta3D::__coef_m<3,1,2,P,1>::job(cc,f,m);
      int KU = __min(1,1);      
      // __coef_k<ORDR,N,M,0,KU, S>::job(c,f,m.template pole<M>());
      //         <3,1,2,2,1>
     symt3D<2,X> p = m.template pole<2>();
     f.template tensor<2>().add_inn_prd(p, c.template tensor<0>());     
     //  __coef_k<ORDR,N,M,K+1,KU,1>::job(c,f,p);	  
     symt3D<2,X> p1 = m.template pole<2>();	    
     f.template tensor<3>().add_inn_prd(p1, c.template tensor<1>());
     //	__coef_m<ORDR,N,M+1,MU,!S>::job(c,f,m);
     //            <3,1,3,2,0>
     KU = __min(1,0);
     //	__coef_k<ORDR,N,M,0,KU, S>::job(c,f,m.template pole<M>());
     //            <3,1,3,0,0,0>
     symt3D<3,X> p3 = m.template pole<3>();
     f.template tensor<3>().sub_inn_prd(p3, c.template tensor<0>());
     c.template tensor<0>() -= cc.template tensor<0>();
     c.template tensor<1>() += cc.template tensor<1>();
