...
extern falcON::OctTree * globalTree;
#endif
...
namespace falcON {
    ...
    public:
        ...
        public:
            ...
            vect POS ;// __attribute__ ((aligned (32)));                                 ///< centre of mass = position
            void*PROP;                                ///< pointer to more data
        };
        
        
        class Leaf : public pfalcONstruct {
#else
#ifdef iGPU
 class pfalcONstruct
        {
...
#endif
#endif
#endif // #if defined(pfalcON) && (! defined(iGPU)) 
	      //#ifndef iGPU
	    ...
	    private:
            ...
            } ;//__attribute__ ((aligned (32))); // class Leaf
            //==========================================================================
            //
        ...
        public:
            ...
            public:
#ifdef iGPU
	      int ID1; // XXX indice du dev dans tableau dev multipole : peut être enlevé ? recalculé ? 
	      int ID2; // XXX indice du dev dans tableau dev locaux : peut être enlevé ? recalculé ? 
#endif
	  ...
	  void unmap_leafs();
	  void unmap_cells();
	  void map_leafs(cl_map_flags map_flags = CL_MAP_READ|CL_MAP_WRITE);  
	  void map_cells(cl_map_flags map_flags = CL_MAP_READ|CL_MAP_WRITE);
#endif
            //--------------------------------------------------------------------------
...
++NAME)                                        // get next child
      ...
      //#ifdef iGPU
      //extern falcON::OctTree * T globalT;
      //#endif
