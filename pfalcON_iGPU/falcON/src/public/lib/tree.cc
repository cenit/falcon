...
inline void OctTree::allocate (unsigned ns, unsigned nc, unsigned dm, real r0) {
 ...
 #ifdef iGPU
    //printf("%d cell\n", nc);
    //printf("%d size of one cell, %d size of cells buffer \n", sizeof(Cell), sizeof(Cell)*nc);
    gpu->create_buffer(gpu->cl_bufs + CLBUF_LEAF, Ns*sizeof(Leaf), (void **) &(LEAFS), CQ_P2P);
    gpu->create_buffer(gpu->cl_bufs + CLBUF_CELL, Nc*sizeof(Cell), (void **) &(CELLS), CQ_M2L);

    RA    = static_cast<real*>(static_cast<void*>( DUINT + 4));
    RA[0] = r0;
...
//------------------------------------------------------------------------------
#ifdef iGPU
void OctTree::unmap_leafs(){
  gpu->unmap_buffer(gpu->cl_bufs + CLBUF_LEAF, LEAFS, CQ_P2P);  
  gpu->cl_bufs_map_ptr[CLBUF_LEAF] = NULL; 
}
void OctTree::unmap_cells(){
  gpu->unmap_buffer(gpu->cl_bufs + CLBUF_CELL, CELLS, CQ_M2L);  
  gpu->cl_bufs_map_ptr[CLBUF_CELL] = NULL; 
}
void OctTree::map_leafs(cl_map_flags map_flags){
  gpu->map_buffer(gpu->cl_bufs + CLBUF_LEAF, Ns*sizeof(Leaf), (void **) &(LEAFS), CQ_P2P, CL_TRUE, map_flags);
  gpu->cl_bufs_map_ptr[CLBUF_LEAF] = LEAFS; 
}
void OctTree::map_cells(cl_map_flags map_flags){
  gpu->map_buffer(gpu->cl_bufs + CLBUF_CELL, Nc*sizeof(Cell), (void **) &(CELLS), CQ_M2L, CL_TRUE, map_flags);
  gpu->cl_bufs_map_ptr[CLBUF_CELL] = CELLS; 
}
#endif
//------------------------------------------------------------------------------
