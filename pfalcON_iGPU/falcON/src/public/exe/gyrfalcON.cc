...
#define falcON_VERSION   "3.6 p0.1" // XXX versions pfalcON à préciser 
#define falcON_VERSION_D "XX-Jui-2016 B. Lange, M. Touche and P. Fortin"
//------------------------------------------------------------------------------
#ifndef falcON_NEMO
...
const char*defv[] = {
    ...
    "lastout=t\n        output for t=tstop?                                ",
#ifdef pfalcON
    "tct=10000\n        task creation threshold (pfalcON)                  ",
    "nbthreads=1\n      number of threads (pfalcON)                        ",
#endif
#ifdef iGPU
    "buf_size=400000\n  size of buffers of interaction                     ",
    "device=0\n         device for OpenCL kernels                          ",
#ifdef iGPU_M2L
    "wg_size_M2L=256\n  work-group size for M2L                            ",
#endif
#ifdef iGPU_P2P
    "wg_size_P2P=256\n  work-group size for P2P                            ",
#endif
#endif
    falcON_DEFV, NULL };
//------------------------------------------------------------------------------
...
//    printf("%d \n",nbSIMD);
  ...
  //  printf("T0 : %zu, T1: %zu\n", sizeof(falcON::GravEstimator::Leaf::acpn_data),sizeof(ispc::pfalcONstructPROP) );
//    printf("T0 : %zu, T1: %zu\n", sizeof(falcON::OctTree::Leaf),sizeof(ispc::pfalcONstruct) );
    
    if(sizeof(falcON::OctTree::Leaf) != ispc::sizeof_pfalcONstruct())
    {
        printf("Error : falcON::OctTree::Leaf hasn't the same size than ispc::pfalcONstruct\n");
	printf("sizeof(falcON::OctTree::Leaf)=%lu \t ispc::sizeof_pfalcONstruct()=%d \n",
	       sizeof(falcON::OctTree::Leaf), ispc::sizeof_pfalcONstruct());
	exit(0);        
    }
    if((uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->POS[0]))) != ispc::shift_pfalcONstruct_pos(0)) 
    {
      printf("T0 : %lu, T1: %lu\n", (uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->POS[0]))), ispc::shift_pfalcONstruct_pos(0));
        printf("Error : position of POS[0] is different\n");
        exit(0);
    }
    if((uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->POS[1]))) != ispc::shift_pfalcONstruct_pos(1)) 
    {
        printf("Error : position of POS[1] is different\n");
        printf("T0 : %lu, T1: %lu\n", (uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->POS[1]))), ispc::shift_pfalcONstruct_pos(1));
        exit(0);
    }
    if((uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->POS[2]))) != ispc::shift_pfalcONstruct_pos(2)) 
    {
        printf("Error : position of POS[2] is different\n");
        printf("T0 : %lu, T1: %lu\n", (uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->POS[2]))) , ispc::shift_pfalcONstruct_pos(2));
        exit(0);
    }
    if((uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->SCAL))) != ispc::shift_pfalcONstruct_SCAL())  
    {
        printf("Error : position of SCAL is different\n");
        printf("T0 : %lu, T1: %lu\n", (uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->SCAL))), ispc::shift_pfalcONstruct_SCAL());
        exit(0);
    }
...
#ifdef pfalcON
    tct = 10000; // default value     
    tct = getuparam  ("tct");
#ifdef pfalcON_useTBB
    task_scheduler_init init(getuparam  ("nbthreads"));
#else
    //OpenMP
    omp_set_num_threads(getuparam  ("nbthreads"));

#endif
#endif
#ifdef iGPU
    //    std::cout << "### pfalcON_iGPU argv[0] = " << argv[0] << std::endl; 
    gpu = new cl_manip((getuparam("device") == 0 ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU), 
#ifdef pfalcON 
		       getuparam("nbthreads"), 
#else
		       1,
		       ...
		       getuparam("buf_size"),
#ifdef iGPU_M2L
		       getuparam("wg_size_M2L"),
#else
		       0,
#endif 
#ifdef iGPU_P2P 
		       getuparam("wg_size_P2P"), 
#else
		       0, 
#endif
		       argc, argv);
    ...
    int nb_fields = 4;
    uint64_t *ptr_sizes = new uint64_t[nb_fields];
    gpu->check_pfalcONstruct_sizes(ptr_sizes,nb_fields);
    if(sizeof(falcON::OctTree::Leaf) != ptr_sizes[3])
    {
        printf("Error : falcON::OctTree::Leaf hasn't the same size than the OpenCL pfalcONstruct\n");
    	printf("sizeof(falcON::OctTree::Leaf)=%lu \t sizeof_pfalcONstruct()=%ld \n",
    	       sizeof(falcON::OctTree::Leaf), ptr_sizes[3]);
    	exit(0);        
    }
    if((uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->SCAL))) != ptr_sizes[0])  
    {
        printf("Error : position of SCAL is different\n");
        printf("T0 : %lu, T1: %lu\n", (uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->SCAL))), ptr_sizes[0]);
        exit(0);
    }
    if((uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->idACPN))) != ptr_sizes[1]) 
    {
        printf("Error : position of idACPN/id in pfalcONstruct is different\n");
        printf("T0 : %lu, T1: %lu\n", (uint64_t) ((&(((falcON::OctTree::pfalcONstruct*)(0))->POS[0]))), ptr_sizes[1]);
        exit(0);
    }
    if((uint64_t) ((&(((falcON::OctTree::Leaf*)(0))->FLAGS))) != ptr_sizes[2])  
      {
        printf("Error : position of FLAGS is different\n");
        printf("T0 : %lu, T1: %lu\n", (uint64_t) ((&(((falcON::OctTree::Leaf*)(0))->FLAGS))), ptr_sizes[2]);
        exit(0);
    }
    delete [] ptr_sizes;
...
#ifdef traceKernels
    printf("PP %d %lf -ML %d  %lf\n",nbPP, tempsPP, nbM2L, tempsM2L);
    for(int i = 0 ; i < 512; i++)
    {
        for(int j = 0 ; j < 512; j++)
        {
