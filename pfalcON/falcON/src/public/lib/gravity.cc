...
#define MASS_WEIGHTED_SOFTENING 1
    ...
    // + also initialize all cell locks to 0 
    LoopCellsDown(grav::cell_iter,TREE,Ci) {       
      if (! Ci->hasCoeffs()){
	grav::Cset*X = new grav::Cset();
	X->set_zero();
	Ci->setCoeffs(X);
      }
    }
...
)
{
    ...
    double t0 = 0;//my_gettimeofday();
    double t1 = 0;//my_gettimeofday();
...
#pragma omp taskwait
    ...
    std::cerr<<" time: pfalcON (elapsed time (s)): interaction = "<< t0<<" & evaluation = "<<t1<<std::endl;
